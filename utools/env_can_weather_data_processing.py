...
def interpolateData(interval, temp, dateTime, windDir = 0, windSpd = 0, press = 0):
    ...
    atmTempInterp = numpy.interp(list(numpy.arange(0, ratio * len(temp))), list(numpy.arange(0, ratio * len(temp), ratio)), temp)
    dateTimeInterp = numpy.interp(list(numpy.arange(0, ratio * len(temp))), list(numpy.arange(0, ratio * len(temp), ratio)), dateTime)
    if windDir != 0:
        windDirInterp = numpy.interp(list(numpy.arange(0, ratio * len(temp))), list(numpy.arange(0, ratio * len(temp), ratio)), windDir)
    if windSpd != 0:
        windSpeedInterp = numpy.interp(list(numpy.arange(0, ratio * len(temp))), list(numpy.arange(0, ratio * len(temp), ratio)), windSpd)
    if press != 0:
        atmPressInterp = numpy.interp(list(numpy.arange(0, ratio * len(temp))), list(numpy.arange(0, ratio * len(temp), ratio)), press)

    ...
    try:
        if span != None:
            ...
            if span == "window_6hour":  # 30 * 6 for a 2 minute sampling
                ...
                return
            results = smooth.smoothfit(dateTime, temp, int(nspan), window)
        else:
            results = {}
            results['smoothed'] = temp
    except IOError as arg:
        print("IO Error:%s" % (arg))
        ifile.close()
...
def read_files_and_display(rpath = None, doprofiles=False, ticklab=None, tickval=None, maxidx=30000 , interp=None):
    if rpath != None:
        ppath = rpath
    else:
        ppath = path
    ...
    display_data.display_img_temperatures(dateTimeArr, tempArr, resultsArr, k, tick, maxdepth, firstlogdepth, maxtemp, \
                                          fontsize = 22, datetype=datetype, cblabel="Temp [$\circ$C]", interp=interp)
    if doprofiles:
...
def plot_upwelling_multiple_locations():
    ...
    k = [21, 17, 31, 44]
    utools.display_data.display_upwelling(dateTime, temp, results, k)

...
def read_hdf_datasets_info(filename):
    try:
        ...
        print("Dataset.attributes' keys: ", list(f.attributes().keys()))
        attNames = list(attr.keys())
        # output: ['HDFEOSVersion', 'act_specific', 'StructMetadata.0',...

        print("Dataset.attributes' values: ", list(f.attributes().values()))
        attValues = list(attr.values())
        # output: ['HDFEOSVersion', 'act_specific', 'StructMetadata.0',...
    except SD.HDF4Error as msg:
        print("HDF4Error", msg)
...
def read_hdf_dir(dirname, var, ix, iy, timeidx, startdate, enddate):
    ...
    base, dirs, files = next(iter(os.walk(dirname)))

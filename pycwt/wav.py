...
def ar1(x):
    ...
    c0 = numpy.dot(x.transpose(), x / N)
    c1 = numpy.dot(x[0:N - 1].transpose(), x[1:N]) / (N - 1)
    ...
    def gammest(gin):
        ...
        gk = numpy.arange(1, N)
        gk = gin ** gk
        mu2 = (1.0 / N) + (2.0 / N ** 2.0) * numpy.sum(numpy.arange(N - 1, 0, -1) *
                                                    gk)
    ...
    try:
        g = optimize.newton(gammest, g0, tol = 0.1, maxiter = 5000)  # was 0.0001
    except:
    ...
    gk = numpy.arange(1, N)
    gk = g ** gk
    mu2 = (1.0 / N) + (1.0 / N ** 2.0) * 2.0 * numpy.sum(numpy.arange(N - 1, 0, -1) *
                                                      gk)
    c0est = c0 / (1.0 - mu2)
    a = numpy.sqrt((1.0 - g ** 2) * c0est)
...
def cwt(signal, dt = 1., dj = 1. / 12, s0 = -1, J = -1, wavelet = Morlet(), result = None):
    ...
    EXAMPLE
        ...
        wave, scales, freqs, coi, ufft, fftfreqs = wavelet.cwt(var,
...
def wct_significance(a1, a2, significance_level = 0.95, mc_count = 300,
    ...
    # Save the results on cache to avoid too many computations in the future
    try:
        makedirs(cached)
    except:
        pass
